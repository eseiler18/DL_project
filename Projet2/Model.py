...
class Model():
    def __init__(self, input_channels=3, output_channels=3,kernel_size = 3,nbchannels1=256,nbchannels2=32,nbchannels3=256) -> None:

        self.model = Sequential(Conv2d(input_channels, nbchannels1, kernel_size=kernel_size,
                                       stride=2, padding=1),#padding=1 if kernel_size=3
                                ReLU(),
                                Conv2d(nbchannels1, nbchannels2, kernel_size=kernel_size, stride=2,
                                       padding=1),
                                ReLU(),
                                NearestUpsampling(2),
                                Conv2d(nbchannels2, nbchannels3, kernel_size=kernel_size, padding=1),
                                ReLU(),
                                NearestUpsampling(2),
                                Conv2d( nbchannels3, output_channels, kernel_size=kernel_size, padding=1),
                                Sigmoid())
